...
contract MockOffchainRelayer {
    ...
    function getPastEncodedSignedVaas(uint16 chainId, uint64 deliveryVAASequence)
        public
        view
    ...
    function relay(Vm.Log[] memory logs, bytes memory deliveryOverrides, bool debugLogging) public {
        ...
        if (checkCCTP) {
            cctpEntries = relayerCircleSimulator.fetchMessageTransmitterLogsFromLogs(logs);
        }
        ...
        for (uint16 i = 0; i < encodedSignedVaas.length; i++) {
            ...
            if (
                ...
                vm.selectFork(forks[chainIdOfWormholeAndGuardianUtilities]);
                genericRelay(
                    encodedSignedVaas[i], encodedSignedVaas, circleSignedMessages, parsed[i], deliveryOverrides
                );
            }
        }
        ...
        if (payloadId == 1) {
            ...
            for (uint8 i = 0; i < instruction.messageKeys.length; i++) {
                if (instruction.messageKeys[i].keyType == 1) {
                    // VaaKey
                    (VaaKey memory vaaKey,) = decodeVaaKey(instruction.messageKeys[i].encodedKey, 0);
                    ...
                    for (uint8 j = 0; j < cctpMessages.length; j++) {
                        if (cctpKeyMatchesCCTPMessage(key, cctpMessages[j])) {
                            encodedSignedVaasToBeDelivered[i] =
                                abi.encode(cctpMessages[j].message, cctpMessages[j].signature);
                            break;
                        }
                    }
                }
