...
//   which is used in the CoreBridge. This allows reducing gas costs of verifying VAAs by decoding
//   the VAA oneself using `decodeVmStructCd` and then invoking the CoreBridge's `verifyVM` function
//   instead of `parseAndVerifyVM` (sadly, there's still a silly [hash check](https://github.com/wormhole-foundation/wormhole/blob/1dbe8459b96e182932d0dd5ae4b6bbce6f48cb09/ethereum/contracts/Messages.sol#L44-L66)
//   in there that can't be disabled).
//
